...
export default function GanttPage() {
  const { data, loading, error, hasImported } = useIpoData();
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <h1 className="text-2xl font-bold text-slate-800">甘特图</h1>
        {hasImported && (
        ...
        <>
          ...
          <div className="text-xs text-slate-400 bg-slate-50 rounded-lg px-4 py-2 border border-slate-200">
            <span className="inline-flex items-center gap-4">
              <span className="flex items-center gap-1"><span className="w-2.5 h-2.5 rounded bg-amber-500" /> 里程碑</span>
              <span className="flex items-center gap-1"><span className="w-2.5 h-2.5 rounded bg-brand-500" /> 事件</span>
